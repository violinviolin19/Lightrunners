...
Room::Room(cugl::Size size)
  ...
  for (float i = 0.0f; i <= size.width * size.height; i++) {
    ...
    tile->setContentSize(cugl::Vec2::ONE - cugl::Vec2(0.2f, 0.2f));
    tile->setAnchor(cugl::Vec2::ANCHOR_BOTTOM_LEFT);
    tile->setPosition(fmod(i, size.width) + 0.1f,
                      floorf(i / size.width) + 0.1f);
    tile->setColor(cugl::Color4(0, 0, 0, 0.1f));
    node->addChild(tile);
  }
