...
void LevelGenerator::connectLayers(std::vector<std::shared_ptr<Room>> &layer_a,
  ...
  std::vector<std::pair<std::shared_ptr<Room>, std::shared_ptr<Room>>>
      connections;

  for (int i = 0; i < num_connections; i++) {
    std::pair<std::shared_ptr<Room>, std::shared_ptr<Room>> winner;
      ...
      if (a_room->type == Room::RoomType::DEFAULT) {
        for (std::shared_ptr<Room> &b_room : layer_b) {
          if (b_room->type == Room::RoomType::DEFAULT &&
            ...
            std::pair<std::shared_ptr<Room>, std::shared_ptr<Room>> curr =
                std::make_pair(a_room, b_room);
            auto res = std::find(connections.begin(), connections.end(), curr);

            if (dist < winner_dist && res == connections.end()) {
              winner_dist = dist;
  ...
  for (std::pair<std::shared_ptr<Room>, std::shared_ptr<Room>> &connection :
       connections) {
    auto edge = std::make_shared<Edge>(connection.first, connection.second);
    connection.first->edges.push_back(edge);
    connection.second->edges.push_back(edge);
    edge->path->setColor(cugl::Color4(15, 15, 230, 147));
  }
}
