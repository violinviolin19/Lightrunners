...
void LevelGenerator::placeRegularRooms(int num_rooms, float min_radius,
  ...
  std::normal_distribution<float> size_dis(9.0f, 3.0f);
...
void LevelGenerator::markAndFillHallways() {
  ...
  std::vector<std::shared_ptr<Room>> inside_circle_rooms;
  std::copy_if(
      _rooms.begin(), _rooms.end(), std::back_inserter(inside_circle_rooms),
      [this](const std::shared_ptr<Room> &room) {
        return room->getMid().length() <= this->_config.getInnerCircleRadius();
      });

  std::vector<std::shared_ptr<Room>> middle_circle_rooms;
  std::copy_if(_rooms.begin(), _rooms.end(),
               std::back_inserter(middle_circle_rooms),
               [this](const std::shared_ptr<Room> &room) {
  ...
  std::vector<std::shared_ptr<Room>> outside_circle_rooms;
  std::copy_if(
      _rooms.begin(), _rooms.end(), std::back_inserter(outside_circle_rooms),
      [this](const std::shared_ptr<Room> &room) {
        return room->getMid().length() > this->_config.getMiddleCircleRadius();
      });

  calculateDelaunayTriangles(inside_circle_rooms, 0.0f);
  calculateDelaunayTriangles(middle_circle_rooms,
                             _config.getInnerCircleRadius());
  calculateDelaunayTriangles(outside_circle_rooms,
                             _config.getMiddleCircleRadius());

  calculateMinimumSpanningTree(inside_circle_rooms);
  calculateMinimumSpanningTree(middle_circle_rooms);
  calculateMinimumSpanningTree(outside_circle_rooms);
...
void LevelGenerator::calculateDelaunayTriangles(
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    if (inner_circle_cutoff_radius == 0.0f ||
        !edge_0_1->doesIntersect(cugl::Vec2::ZERO,
    ...
    if (inner_circle_cutoff_radius == 0.0f ||
        !edge_0_2->doesIntersect(cugl::Vec2::ZERO,
                                 inner_circle_cutoff_radius)) {
    ...
    if (inner_circle_cutoff_radius == 0.0f ||
        !edge_1_2->doesIntersect(cugl::Vec2::ZERO,
                                 inner_circle_cutoff_radius)) {
