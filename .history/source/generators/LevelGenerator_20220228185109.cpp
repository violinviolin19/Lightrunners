...
void LevelGenerator::connectLayers(std::vector<std::shared_ptr<Room>> &layer_a,
  ...
  std::uniform_real_distribution<float> dis(0.0f, 1.0f);
  float min_angle = dis(_generator) * 2 * M_PI;
  float max_angle = fmod(min_angle + M_PI / 2.0f, 2 * M_PI);
  ...
  for (int i = 0; i < num_connections; i++) {
    ...
    for (std::shared_ptr<Room> &a_room : layer_a) {
      float angle = a_room->getMid().getAngle() + M_PI / 2.0f;
      if (angle >= min_angle && angle <= max_angle &&
          a_room->type == Room::RoomType::DEFAULT) {
    ...
    if (winner) {
      ...
      max_angle = fmod(max_angle + 2 * M_PI / num_connections, 2 * M_PI);
    }
  }
