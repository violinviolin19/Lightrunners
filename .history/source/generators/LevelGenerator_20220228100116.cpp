...
std::shared_ptr<Room>
LevelGenerator::roomMostOverlappingWith(std::shared_ptr<Room> room) {
...
void LevelGenerator::calculateDelaunayTriangles(
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    if (!edge_0_1.doesInterect(cugl::Vec2::ZERO), inner_circle_cutoff_radius) {
    ...
    if (!edge_0_2.doesInterect(cugl::Vec2::ZERO), inner_circle_cutoff_radius) {
    ...
    if (!edge_1_2.doesInterect(cugl::Vec2::ZERO), inner_circle_cutoff_radius) {
