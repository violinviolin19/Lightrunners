...
std::shared_ptr<Edge> Room::getEdge(const std::shared_ptr<Room> room) const {
  auto result = std::find_if(
      edges.begin(), edges.end(), [room](const std::shared_ptr<Edge> &edge) {
        return edge->source == room || edge->neighbor == room;
      });
...
void LevelGenerator::markAndFillHallways() {
  ...
  for (auto room : _rooms) {
    CULog("numEdges: %d", room->edges.size());
  }
...
void LevelGenerator::calculateDelaunayTriangles(
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    if (r == 0.0f || !edge_0_1->doesIntersect(cugl::Vec2::ZERO, r)) {
      ...
      bool adjacent_0_1 = node_0->hasNeighbor(node_1);
      bool adjacent_1_0 = node_1->hasNeighbor(node_0);
    ...
    if (r == 0.0f || !edge_0_2->doesIntersect(cugl::Vec2::ZERO, r)) {
      bool adjacent_0_2 = node_0->hasNeighbor(node_2);
      bool adjacent_2_0 = node_2->hasNeighbor(node_0);
    ...
    if (r == 0.0f || !edge_1_2->doesIntersect(cugl::Vec2::ZERO, r)) {
      bool adjacent_1_2 = node_1->hasNeighbor(node_2);
      bool adjacent_2_1 = node_2->hasNeighbor(node_1);

      if (!adjacent_1_2 && !adjacent_2_1)
        _map->addChild(edge_1_2->path);

      if (!adjacent_1_2)
        node_0->edges.push_back(edge_1_2);
