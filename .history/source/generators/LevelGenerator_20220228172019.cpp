...
bool EdgeEquals::operator()(const std::shared_ptr<Edge> &edge_1,
                            const std::shared_ptr<Edge> &edge_2) const {
...
void LevelGenerator::markAndFillHallways() {
  ...
  // segregateLayer(_inside_rooms, 0.0f, _config.getInnerCircleRadius());
  // segregateLayer(_middle_rooms, _config.getInnerCircleRadius(),
  //                _config.getMiddleCircleRadius());
  // segregateLayer(_outside_rooms, _config.getMiddleCircleRadius(),
  //                _config.getMapRadius());
...
void LevelGenerator::calculateDelaunayTriangles() {
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    std::shared_ptr<Edge> adjacent_0_1 = node_0->findEdge(node_1);
    std::shared_ptr<Edge> adjacent_1_0 = node_1->findEdge(node_0);
    ...
    std::shared_ptr<Edge> adjacent_0_2 = node_0->findEdge(node_2);
    std::shared_ptr<Edge> adjacent_2_0 = node_2->findEdge(node_0);
    ...
    std::shared_ptr<Edge> adjacent_1_2 = node_1->findEdge(node_2);
    std::shared_ptr<Edge> adjacent_2_1 = node_2->findEdge(node_1);
...
void LevelGenerator::segregateLayer(std::vector<std::shared_ptr<Room>> &layer,
                                    float min_r, float max_r) {
  std::vector<std::shared_ptr<Room>> inside;
  std::copy_if(_rooms.begin(), _rooms.end(), std::back_inserter(inside),
               [min_r, max_r](const std::shared_ptr<Room> &room) {
                 float r = room->getMid().length();
                 return room->type == Room::RoomType::DEFAULT && r > min_r &&
                        r <= max_r;
               });
  layer.insert(layer.end(), inside.begin(), inside.end());

  for (std::shared_ptr<Room> &room : layer) {
    for (std::shared_ptr<Edge> &edge : room->edges) {
      if (min_r == 0.0f || !edge->doesIntersect(cugl::Vec2::ZERO, min_r)) {
        edge->active = false;
