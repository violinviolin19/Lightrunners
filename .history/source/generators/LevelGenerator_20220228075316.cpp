...
LevelGenerator::roomMostOverlappingWith(std::shared_ptr<Room> room) {
  std::vector<std::shared_ptr<Room>> intersecting;
  cugl::Rect room_rect = room->getRect();

  for (std::shared_ptr<Room> n_room : _rooms) {
    cugl::Rect n_room_rect = n_room->getRect();

    if (room ! = n_room && room_rect.doesIntersect(n_room_rect)) {
      intersecting.push_back(n_room);
    }
  }
...
void LevelGenerator::placeTerminalRooms(int num_rooms, float min_radius,
  ...
  for (float i = 0; i < num_rooms; i++) {
    ...
    for (std::shared_ptr<Room> n_room : roomsOverlappingWith(room)) {
      n_room->node->setColor(cugl::Color4(205, 14, 14, 127));
    }
  }
}
