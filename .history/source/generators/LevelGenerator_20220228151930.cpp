...
bool Room::isNeighbor(const std::shared_ptr<Room> room) const {
  auto result =
      std::find_if(a_room->edges.begin(), a_room->edges.end(),
                   [b_room](const std::shared_ptr<Edge> &edge) {
                     return edge->source == b_room || edge->neighbor == b_room;
                   });

  if (result != a_room->edges.end()) {
    cugl::Vec2 a_mid = b_room->getMid();
  }
}
...
void LevelGenerator::connectLayers(
  ...
  for (std::shared_ptr<Room> &a_room : layer_a) {
    ...
    for (std::shared_ptr<Room> &b_room : layer_b) {
      // Find if b_room is connected to a_room.
    }
  }
}
