...
void LevelGenerator::markAndFillHallways() {
  ...
  std::vector<std::shared_ptr<Rooms>> inside_circle;
  std::copy_if(_rooms.begin(), _rooms.end(), std::back_inserter(inside_circle),
               [this](std::shared_ptr<Rooms> room) {
                 return room->getRect().doesInclude(
                     cugl::Vec2::ZERO, this->_config.getInnerCircleRadius());
               });
