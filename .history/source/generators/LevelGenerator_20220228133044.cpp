...
void LevelGenerator::generateRooms() {
  ...
  _spawn_room = std::make_unique<Room>(LevelGenerator::SPAWN);
...
void LevelGenerator::calculateDelaunayTriangles(
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    std::shared_ptr<Room> edge_0_1 = std::make_shared<Edge>(node_0, node_1);
    if (!edge_0_1->doesIntersect(cugl::Vec2::ZERO,
                                 inner_circle_cutoff_radius)) {
      _map->addChild(edge_0_1->path);
      node_0->edges.push_back(std::move(edge_0_1));
      node_1->edges.push_back(std::move(edge_0_1));
    }

    std::shared_ptr<Room> edge_0_2 = std::make_shared<Edge>(node_0, node_2);
    if (!edge_0_2->doesIntersect(cugl::Vec2::ZERO,
                                 inner_circle_cutoff_radius)) {
      _map->addChild(edge_0_2.path);
      node_0->edges.push_back(std::move(edge_0_2));
      node_2->edges.push_back(std::move(edge_0_2));
    }

    std::shared_ptr<Room> edge_1_2 = std::make_shared<Edge>(node_1, node_2);
    if (!edge_1_2->doesIntersect(cugl::Vec2::ZERO,
                                 inner_circle_cutoff_radius)) {
      _map->addChild(edge_1_2.path);
      node_1->edges.push_back(std::move(edge_1_2));
      node_2->edges.push_back(std::move(edge_1_2));
    }
  }
...
void LevelGenerator::calculateMinimumSpanningTree() {
  ...
  while (num_edges < _rooms.size() - 1) {
    std::unique_ptr<Edge> min_edge = nullptr;

    for (Room &room : _rooms) {
      if (room.visited) {
        for (Edge &edge : room.edges) {
          if (!edge.neighbor->visited && edge.weight < min_edge->weight) {
            min_edge.release();
            min_edge.reset(&edge);
          }
        }
