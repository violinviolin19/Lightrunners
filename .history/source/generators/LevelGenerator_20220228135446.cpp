...
void LevelGenerator::markAndFillHallways() {
  ...
  calculateDelaunayTriangles(inside_circle, 0.0f);
  calculateDelaunayTriangles(middle_circle, _config.getInnerCircleRadius());
  calculateDelaunayTriangles(outside_circle, _config.getMiddleCircleRadius());

  calculateMinimumSpanningTree(inside_circle);
  calculateMinimumSpanningTree(middle_circle);
  calculateMinimumSpanningTree(outside_circle);
  addEdgesBack();
  fillHallways();
