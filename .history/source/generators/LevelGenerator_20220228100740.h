...
struct Edge {
  ...
  bool doesIntersect(cugl::Vec2 origin, float radius) {
    cugl::Vec2 A = source->getMid();
    cugl::Vec2 B = source->getMid();

    cugl::Vec2 AB = neighbor->getMid() - source->getMid();
    cugl::Vec2 AC = origin - source->getMid();
    cugl::Vec2 D = AC.getProjection(AB) + source->getMid();

    return DC.length() < radius;
  }
};
