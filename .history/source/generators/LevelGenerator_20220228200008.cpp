...
void LevelGenerator::segregateLayers() {
  ...
  for (std::shared_ptr<Room> &room : _middle_rooms) {
    cugl::Vec2 disp = room->getMid() * 1.5f - room->node->getSize() / 2.0f;
    dir.set(roundf(dir.x), roundf(dir.y));
    room->node->setPosition(dir);
  }

  for (std::shared_ptr<Room> &room : _outside_rooms) {
    cugl::Vec2 dir = room->getMid() * 1.5f - room->node->getSize() / 2.0f;
    dir.set(roundf(dir.x), roundf(dir.y));
    room->node->setPosition(dir - room->node->getSize() / 2.0f);
  }
