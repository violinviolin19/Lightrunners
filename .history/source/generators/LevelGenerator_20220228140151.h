...
private:
  ...
  std::vector<std::shared_ptr<Room>> _inner_rooms;
  std::vector<std::shared_ptr<Room>> _middle_rooms;
  std::vector<std::shared_ptr<Room>> _outer_rooms;
...
private:
  ...
  std::vector<std::shared_ptr<Room>>
  placeRegularRooms(int num_rooms, float min_radius, float max_radius,
                    std::default_random_engine &generator);
