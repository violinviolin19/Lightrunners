...
void LevelGenerator::calculateDelaunayTriangles(
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    if (inner_cutoff_radius == 0.0f ||
      ...
      auto r = node_0->edges.insert(edge_0_1);
      number_of_fails += r.second;
      // CULog("node 0 -> edge 0 1 : %d", r.second);
      r = node_1->edges.insert(edge_0_1);
      // CULog("node 1 -> edge 0 1 : %d", r.second);
      number_of_fails += r.second;
    }
    ...
    if (inner_cutoff_radius == 0.0f ||
      ...
      auto r = node_0->edges.insert(edge_0_2);
      // CULog("node 0 -> edge 0 2 : %d", r.second);
      number_of_fails += r.second;
      r = node_2->edges.insert(edge_0_2);
      // CULog("node 2 -> edge 0 2 : %d", r.second);
      number_of_fails += r.second;
    }
    ...
    if (inner_cutoff_radius == 0.0f ||
      ...
      auto r = node_1->edges.insert(edge_1_2);
      // CULog("node 1 -> edge 1 2 : %d", r.second);
      number_of_fails += r.second;
      r = node_2->edges.insert(edge_1_2);
      // CULog("node 2 -> edge 1 2 : %d", r.second);
      number_of_fails += r.second;
    }
  }
  CULog("%d", number_of_fails);
  _map->doLayout();
}
