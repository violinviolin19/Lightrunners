...
struct Edge {
  ...
  bool doesIntersect(cugl::Vec2 origin, float radius) {
    ...
    cugl::Vec2 s_pos = source->getMid();
    cugl::Vec2 n_pos = neighbor->getMid();

    cugl::Vec2 AB = n_pos - s_pos;
    cugl::Vec2 AC = origin - s_pos;
    float area2 = AB.cross(AC);
    float height = area2 * invSqrt(AB.dot(AB));

    return abs(a * origin.x + b * origin.y + c) / sqrtf(a * a + b * b) <=
           radius;
  }
};
