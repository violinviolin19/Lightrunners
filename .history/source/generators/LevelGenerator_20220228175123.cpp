...
void LevelGenerator::connectLayers(std::vector<std::shared_ptr<Room>> &layer_a,
  ...
  std::vector<std::pair<std::shared_ptr<Room>, std::shared_ptr<Room>>> winners;
          ...
          if (b_room->type == Room::RoomType::DEFAULT &&
            ...
            auto curr =
                std::make_pair<std::shared_ptr<Room>, std::shared_ptr<Room>>(
                    a_room, b_room);
            auto res = std::find(winner.begin(), winner.end(), curr);

            if (dist < winner_dist && res == winner.end()) {
              winner_dist = dist;
              winner.first = a_room;
              winner.second = b_room;
            }
          }
        }
      }
  ...
  for (std::pair<std::shared_ptr<Room>, std::shared_ptr<Room>> &winner :
       winners) {
    auto edge = std::make_shared<Edge>(winner.first, winner.second);
    winner.first->edges.push_back(edge);
    winner.second->edges.push_back(edge);
