...
void LevelGenerator::segregateLayer(std::vector<std::shared_ptr<Room>> &layer,
  ...
  for (std::shared_ptr &room : _rooms) {

    for (std::shared_ptr &n_room : _rooms) {
      std::shared_ptr<Edge> edge = room->findEdge(n_room);
      if (r == 0.0f || !edge->doesIntersect(cugl::Vec2::ZERO, r)) {
        edge->active = false;
