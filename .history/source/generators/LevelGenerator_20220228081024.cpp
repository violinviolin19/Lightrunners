...
LevelGenerator::roomMostOverlappingWith(std::shared_ptr<Room> room) {
  ...
  for (std::shared_ptr<Room> n_room : _rooms) {
    ...
    if (room != n_room &&
        n_room_rect.intersect(room_rect).size > overlap_rect.size) {
      overlap_rect = n_room_rect;
      overlap = n_room;
    }
  }
...
void LevelGenerator::placeTerminalRooms(int num_rooms, float min_radius,
  ...
  for (float i = 0; i < num_rooms; i++) {
    ...
    if (overlapping) {
      overlapping->node->setColor(cugl::Color4(52, 205, 14, 127));
    }
  }
}
