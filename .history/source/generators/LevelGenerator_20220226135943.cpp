...
void LevelGenerator::generateRooms() {
  ...
  int num_regular_rooms = _config.getNumRooms() - _config.getNumTerminalRooms();
  for (float i = 0; i < num_regular_rooms; i++) {
    auto room =
        std::make_shared<Room>(floor(size_dis(gen)), floor(size_dis(gen)));
    room->node->setAnchor(cugl::Vec2::ANCHOR_BOTTOM_LEFT);

    float x = dis(gen) * _config.getMapRadius();
    float clamp_y = _config.getMapRadius() * _config.getMapRadius() - x * x;
    float y = dis(gen) * 2 * clamp_y - clamp_y;
    cugl::Vec2 pos(x, y);

    room->node->setPosition(pos - (room->node->getContentSize() / 2.0f));
    room->node->setColor(cugl::Color4(102, 57, 17, 127));
