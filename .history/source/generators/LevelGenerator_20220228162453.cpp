...
void LevelGenerator::calculateDelaunayTriangles(
    std::vector<std::shared_ptr<Room>> &rooms, float rad) {
  ...
  for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
    ...
    if (rad == 0.0f || !edge_0_1->doesIntersect(cugl::Vec2::ZERO, rad)) {
    ...
    if (rad == 0.0f || !edge_0_2->doesIntersect(cugl::Vec2::ZERO, rad)) {
    ...
    if (rad == 0.0f || !edge_1_2->doesIntersect(cugl::Vec2::ZERO, rad)) {
