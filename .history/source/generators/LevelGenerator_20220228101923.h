...
struct Edge {
  ...
  bool doesIntersect(cugl::Vec2 origin, float radius) {
    ...
    float a = s_pos.y - n_pos.y;
    float b = n_pos.x - s_pos.x;
    float c = (s_pos.x - n_pos.x) * s_pos.y + s_pos.y * (n_pos.y - s_pos.y);

    return abs(a * origin.x + b * origin.y + c) / sqrt(a * a + b * b) <= radius;
  }
};
