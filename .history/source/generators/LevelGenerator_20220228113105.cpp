...
void LevelGenerator::placeRegularRooms(int num_rooms, float min_radius,
  ...
  for (float i = 0; i < num_rooms; i++) {
    auto room = Room(floor(size_dis(generator)), floor(size_dis(generator)));
...
Room LevelGenerator::roomMostOverlappingWith(Room &room) const {
  cugl::Rect room_rect = room.getRect();

  auto it = std::max_element(
      _rooms.begin(), _rooms.end(), [](Room const &lhs, Room const &rhs) {
        cugl::Size l_size = room_rect.intersect(lhs.getRect()).size;
        cugl::Size r_size = room_rect.intersect(rhs.getRect()).size;
