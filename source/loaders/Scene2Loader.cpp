 ...
 */
bool Scene2Loader::init(const std::shared_ptr<cugl::ThreadPool>& threads) {
  _loader = threads;
 ...
 */
std::shared_ptr<cugl::scene2::SceneNode> Scene2Loader::build(
  ...
  std::shared_ptr<cugl::JsonValue> data = json->get("data");
  std::shared_ptr<cugl::scene2::SceneNode> node = nullptr;
  switch (it->second) {
    case Widget::NODE:
      node = cugl::scene2::SceneNode::allocWithData(this, data);
      break;
    case Widget::IMAGE:
      node = cugl::scene2::PolygonNode::allocWithData(this, data);
      break;
    case Widget::SOLID:
    ...
    case Widget::POLY:
      ...
      break;
    case Widget::PATH:
      node = cugl::scene2::PathNode::allocWithData(this, data);
      break;
    case Widget::WIRE:
      node = cugl::scene2::WireNode::allocWithData(this, data);
      break;
    case Widget::ANIMATE:
      node = cugl::scene2::SpriteNode::allocWithData(this, data);
      break;
    case Widget::NINE:
      node = cugl::scene2::NinePatch::allocWithData(this, data);
      break;
    case Widget::LABEL:
      node = cugl::scene2::Label::allocWithData(this, data);
      break;
    case Widget::BUTTON:
      node = cugl::scene2::Button::allocWithData(this, data);
      break;
    case Widget::PROGRESS:
      node = cugl::scene2::ProgressBar::allocWithData(this, data);
      break;
    case Widget::SLIDER:
      node = cugl::scene2::Slider::allocWithData(this, data);
      break;
    case Widget::SCROLL:
      node = cugl::scene2::ScrollPane::allocWithData(this, data);
      break;
    case Widget::TEXTFIELD:
      node = cugl::scene2::TextField::allocWithData(this, data);
      break;
    case Widget::EXTERNAL_IMPORT: {
      const std::shared_ptr<cugl::JsonValue> widgetJson = getWidgetJson(json);
  ...
  auto kt = _tile_types.find(cugl::strtool::tolower(type));
  ...
  std::shared_ptr<cugl::JsonValue> form = json->get("format");
  std::string ftype =
      (form == nullptr ? UNKNOWN_STR : form->getString("type", UNKNOWN_STR));
  auto jt = _forms.find(cugl::strtool::tolower(ftype));

  std::shared_ptr<cugl::scene2::Layout> layout = nullptr;
  if (jt != _forms.end()) {
    switch (jt->second) {
      case Form::ANCHORED:
        layout = cugl::scene2::AnchoredLayout::allocWithData(form);
        break;
      case Form::FLOAT:
        layout = cugl::scene2::FloatLayout::allocWithData(form);
        break;
      case Form::GRID:
        layout = cugl::scene2::GridLayout::allocWithData(form);
        break;
      case Form::NONE:
      case Form::UNKNOWN:
        break;
  ...
  std::shared_ptr<cugl::JsonValue> children = json->get("children");
  if (children != nullptr) {
    for (int ii = 0; ii < children->size(); ii++) {
      std::shared_ptr<cugl::JsonValue> item = children->get(ii);
      ...
      if (key != "comment") {
        ...
        std::shared_ptr<cugl::scene2::SceneNode> kid = build(key, item);
        if (nonrelative) {
        ...
        if (layout != nullptr && item->has("layout")) {
          std::shared_ptr<cugl::JsonValue> posit = item->get("layout");
 ...
 *
 * If this scene is built before the JSON of any used widgets have been loaded,
 * this will fail.
 *
 * @param json      The JSON object specifying the widget's file and the values
 * for its exposed variables
 *
 * @return the JSON loaded from the widget file with all variables set based on
 * the values presented in json.
 */
std::shared_ptr<cugl::JsonValue> Scene2Loader::getWidgetJson(
    const std::shared_ptr<cugl::JsonValue>& json) const {
  std::shared_ptr<cugl::JsonValue> data = json->get("data");
  std::string widgetSource = data->getString("key");
  std::shared_ptr<cugl::JsonValue> widgetVars = data->get("variables");
  std::shared_ptr<cugl::JsonValue> layout = json->get("layout");

  const std::shared_ptr<WidgetValue> widget =
      _manager->get<WidgetValue>(widgetSource);

  CUAssertLog(widget != nullptr, "No widget found with name %s",
              widgetSource.c_str());

  const std::shared_ptr<cugl::JsonValue> widgetJson = widget->getJson();

  std::shared_ptr<cugl::JsonValue> variables = widgetJson->get("variables");
  std::shared_ptr<cugl::JsonValue> contents = widgetJson->get("contents");
  std::string contentString = contents->toString();
  std::shared_ptr<cugl::JsonValue> contentCopy =
      cugl::JsonValue::allocWithJson(contentString);
  if (widgetVars) {
    for (int ii = 0; ii < widgetVars->size(); ii++) {
      auto child = widgetVars->get(ii);
      if (variables->has(child->key())) {
        bool found = true;
        std::shared_ptr<cugl::JsonValue> address = variables->get(child->key());
        std::shared_ptr<cugl::JsonValue> spotToChange = contentCopy;
        ...
        if (found) {
          ...
          std::string err = "No variable found within widget " + widgetSource +
                            " matching name " + child->key();
  ...
  if (layout != nullptr) {
    std::shared_ptr<cugl::JsonValue> contentsLayout =
        contentCopy->get("layout");
    if (contentsLayout == nullptr) {
      contentCopy->appendChild("layout", std::make_shared<cugl::JsonValue>());
...
bool Scene2Loader::read(const std::string key, const std::string source,
                        cugl::LoaderCallback callback, bool async) {
  ...
  if (_loader == nullptr || !async) {
    ...
    std::shared_ptr<cugl::JsonValue> json =
        (reader == nullptr ? nullptr : reader->readJson());
    std::shared_ptr<cugl::scene2::SceneNode> node = build(key, json);
    node->doLayout();
    if (node != nullptr) {
      success = true;
 ...
 * @return true if the asset was successfully loaded
 */
bool Scene2Loader::read(const std::shared_ptr<cugl::JsonValue>& json,
                        cugl::LoaderCallback callback, bool async) {
 ...
 */
bool Scene2Loader::purge(const std::shared_ptr<cugl::JsonValue>& json) {
  return false;
}
 ...
 */
bool Scene2Loader::attach(
    const std::string& key,
    const std::shared_ptr<cugl::scene2::SceneNode>& node) {
  _assets[key] = node;

  // Check if class is a tile.
  auto it = _tile_types.find(cugl::strtool::tolower(node->getClassName()));
  if (it != _tile_types.end() && it->second != Tile::BASIC_TILE) {
    _tile_box2d.push_back(
        std::dynamic_pointer_cast<cugl::physics2::BoxObstacle>(node));
  }

  bool success = true;
  for (int ii = 0; ii < node->getChildren().size(); ii++) {
    std::shared_ptr<cugl::scene2::SceneNode> item = node->getChild(ii);
