...
#pragma mark Setters
   ...
   */
  Rect &operator=(const float *array) { return set(array); }
...
#pragma mark Comparisons
  ...
  bool equals(const Rect rect, float variance = CU_MATH_EPSILON) const {
    return origin.equals(rect.origin, variance) &&
           size.equals(rect.size, variance);
  }
   ...
   */
  bool operator<=(const Rect rect) const { return inside(rect); }
   ...
   */
  bool operator>=(const Rect rect) const { return contains(rect); }
...
#pragma mark Operators
   ...
   *
   * The size of the rectangle is unaffected. To alter the rectangle size, scale
   * the rectangle.
   *
   ...
   *
   * @return This rectangle, as the bounding box of the transformed
   * quadrilateral.
   */
   ...
   *
   * The size of the rectangle is unaffected.  To alter the rectangle size,
   * scale the rectangle.
   *
   * @param vec   The translation vector
   *
   ...
   */
  friend Rect operator*(const Vec2 scale, const Rect rect) {
    return rect * scale;
  }
...
#pragma mark Rectangle Arithmetic
  ...
  /**
   * Computes the intersection of this rect and the other, assigning it in
   * place.
   *
